...
import { View, Text, TextInput, TouchableOpacity, Dimensions, StyleSheet, Keyboard, KeyboardAvoidingView, Platform, ScrollView } from "react-native";
import { StatusBar } from "expo-status-bar";
import { LinearGradient } from "expo-linear-gradient";
import Animated, { useSharedValue, useAnimatedStyle, withRepeat, withTiming, withDelay, withSequence, FadeInUp, FadeOutUp, withSpring, runOnJS } from "react-native-reanimated";
...
const floatingWords = [
    ...
    "पथ", "यात्रा", "क्षण", "लम्हा", "अनंत", "सफ़र", "मिलन", "वियोग", "आहट", "नज़दीक"
];
...
export default function LoginScreen() {
    ...
    const handlePasswordLogin = async () => {
        ...
        setError("");
        try {
            const response = await fetch(`https://alfaaz-backend.vercel.app/api/auth/login`, {
                method: "POST",
    ...
    const handleSendOtp = async () => {
        ...
        try {
            const response = await fetch(`https://alfaaz-backend.vercel.app/api/auth/login-otp-request`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    const handleVerifyOtp = async () => {
        ...
        try {
            const response = await fetch(`https://alfaaz-backend.vercel.app/api/auth/login-otp-verify`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                    ...
                    >
                        <View className="w-full bg-white/10 p-6 rounded-3xl border border-white/10" style={{ backdropFilter: 'blur(20px)' }}>
                            <View className="items-center mb-8">
                                <Text className="text-5xl font-bold text-amber-400">
                                    अल्फ़ाज़
                                </Text>
                            ...
                            <View className="space-y-4 gap-4">
                                <TextInput
                                    ...
                                    className={`w-full bg-black/30 text-white p-4 rounded-xl border border-white/10 focus:border-amber-400 ${otpSent ? "opacity-50" : ""}`}
                                ...
                                {useOtp ? (
                                    otpSent && (
                                        <View className="relative">
                                            <TextInput
                                                ...
                                                maxLength={6}
                                                className="w-full bg-black/30 text-white p-4 rounded-xl border border-white/10 focus:border-amber-400 pl-12"
                                                value={otp}
                                                onChangeText={setOtp}
                                            />
                                    ...
                                    <View className="relative">
                                        <TextInput
                                            ...
                                            className="w-full bg-black/30 text-white p-4 rounded-xl border border-white/10 focus:border-amber-400 pr-12"
                                ...
                                <TouchableOpacity
                                    onPress={useOtp ? (otpSent ? handleVerifyOtp : handleSendOtp) : handlePasswordLogin}
                                    disabled={loading}
                                    className="w-full py-4 rounded-xl bg-amber-400 items-center justify-center mt-4 shadow-lg shadow-amber-500/30"
                                >
                                    <Text className="text-black font-semibold text-lg">
                                        {loading ? "Processsing..." : (useOtp ? (otpSent ? "Verify & Login" : "Send Login Code") : "Sign In")}
                                    </Text>
                                </TouchableOpacity>

                                {otpSent && (
                                    <TouchableOpacity onPress={() => setOtpSent(false)} className="items-center">
                                        <Text className="text-amber-400 text-sm">Change Email / Resend</Text>
                                    </TouchableOpacity>
                                )}
                            </View>

                            <View className="mt-6 items-center space-y-4 gap-4">
                                ...
                                <View className="flex-row">
                                    ...
                                    <TouchableOpacity onPress={() => navigation.navigate("Signup")}>
                                        <Text className="text-amber-400">Create an account</Text>
                                    </TouchableOpacity>
                                </View>
                            </View>
                        </View>
